...
const Overview = ({ userData, totalUsers, activeBins, binRequests, reportedIssues, setSelectedView, unreadNotifications }) => {
  ...
  // Filter only pending (unresolved) issues
  const pendingIssues = reportedIssues?.filter(issue => 
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-800 mb-1">Pending Issues</h3>
            <p className="text-3xl font-bold text-red-600">{pendingIssuesCount}</p>
            <p className="text-sm text-gray-500 mt-2">
              {pendingIssuesCount === 0 ? (
                <span className="text-green-600 font-medium">✓ No pending issues</span>
              ) : (
                <>
                  <span className="text-red-600 font-medium">
                    {pendingIssuesCount} {pendingIssuesCount === 1 ? 'issue' : 'issues'} unresolved
                  </span>
                  {reportedIssues?.length > pendingIssuesCount && (
                    <span className="text-gray-400 text-xs block mt-1">
                      {reportedIssues.length - pendingIssuesCount} resolved
                    </span>
                  )}
                </>
              )}
